...
export default function KanbanView({
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white dark:bg-[#191919] text-[#37352F] dark:text-[#D3D3D0] overflow-hidden select-none font-sans">
      {/* 1. NOTION DATABASE TOP HEADER (BANNER + TITLE + VIEW TABS) */}
      <div className="shrink-0 px-6 pt-6 pb-2 border-b border-[#EDEDEB] dark:border-[#2D2D2D] bg-white dark:bg-[#191919]">
        {/* Project Icon & Title */}
        <div className="flex items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-3 min-w-0">
            {/* Notion Monogram / App Icon */}
            <div className="w-10 h-10 rounded-xl bg-black flex items-center justify-center shadow-xs shrink-0 cursor-pointer">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M4 14C4 9.58172 7.58172 6 12 6C16.4183 6 20 9.58172 20 14V17C20 18.6569 18.6569 20 17 20H7C5.34315 20 4 18.6569 4 17V14Z" stroke="white" strokeWidth="2.2" strokeLinecap="round" />
                <path d="M8 12C9.5 9 14.5 9 16 12" stroke="white" strokeWidth="2.2" strokeLinecap="round" />
              </svg>
            </div>
            ...
            <div className="min-w-0 flex-1">
              {isEditingTitle ? (
                <input
                  ...
                  className="text-2xl font-bold text-[#111] dark:text-[#FFF] bg-transparent border-b border-[#2383E2] outline-none px-0 py-0 w-full"
                  autoFocus
                />
              ) : (
                <h1 
                  onClick={() => setIsEditingTitle(true)}
                  className="text-2xl font-bold tracking-tight text-[#111] dark:text-[#FFF] truncate cursor-pointer hover:bg-[#F0F0EE] dark:hover:bg-[#252525] px-1 py-0.5 -ml-1 rounded transition-colors"
          ...
          {/* Right Top Action Controls (Notion Search, Views, Fullscreen, New) */}
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-36 focus:w-48 transition-all pl-8 pr-2.5 py-1 text-[12.5px] rounded-md bg-[#F7F7F5] dark:bg-[#252525] border border-transparent focus:border-[#2383E2] focus:bg-white dark:focus:bg-[#1E1E1E] text-[#37352F] dark:text-[#FFF] outline-none placeholder-[#9B9A97]"
              />
              {localSearch && (
                <button
                  type="button"
                  onClick={() => setLocalSearch('')}
                  className="absolute right-2 text-[#9B9A97] hover:text-[#37352F]"
                >
                  <X className="w-3 h-3" />
                </button>
              )}
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[12.5px] font-medium border transition-colors cursor-pointer ${
                filterStatus !== 'all' || filterPriority !== 'all' || filterTag !== 'all' || filterDueDate !== 'all'
                  ? 'bg-[#EBF5FB] border-[#C5DDE8] text-[#2383E2]'
                  : 'bg-transparent hover:bg-[#EFEFED] dark:hover:bg-[#2F2F2F] border-transparent text-[#787774] dark:text-[#9B9A97]'
              }`}
            >
            ...
            <button
              type="button"
              onClick={() => {
                const nextSort = sortBy === 'default' ? 'priority' : sortBy === 'priority' ? 'dueDate' : 'default';
                setSortBy(nextSort);
              }}
              className="flex items-center gap-1 px-2.5 py-1 rounded-md text-[12.5px] font-medium hover:bg-[#EFEFED] dark:hover:bg-[#2F2F2F] text-[#787774] dark:text-[#9B9A97] transition-colors cursor-pointer"
            >
              <ArrowUpDown className="w-3.5 h-3.5" />
              <span>Sort ({sortBy === 'priority' ? 'Priority' : sortBy === 'dueDate' ? 'Date' : 'Default'})</span>
            </button>
            ...
            <button
              ...
              className="p-1.5 rounded-md hover:bg-[#EFEFED] dark:hover:bg-[#2F2F2F] text-[#787774] dark:text-[#9B9A97] transition-colors cursor-pointer"
            ...
            <div className="relative flex items-center">
              <button
                type="button"
                onClick={() => onCreateTask('New Task', [], 'medium')}
                className="flex items-center gap-1 px-3 py-1 bg-[#2383E2] hover:bg-[#1D70C2] text-white rounded-md text-[13px] font-medium shadow-xs transition-colors cursor-pointer"
              >
                <span>New</span>
              </button>
            </div>
        ...
        {/* Notion View Sub-Tabs (Flow, Priority, Budget, + Project Health, Timeline) */}
        <div className="flex items-center gap-1 -mb-[9px] overflow-x-auto scrollbar-none">
          <button
            type="button"
          ...
          {/* Quick link to switch views to Table or Gantt */}
          {setViewMode && (
            <>
              <button
                type="button"
                onClick={() => setViewMode('table')}
                className="flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-medium border-b-2 border-transparent text-[#787774] dark:text-[#9B9A97] hover:text-[#37352F] dark:hover:text-[#FFF] hover:bg-[#F7F7F5] dark:hover:bg-[#222] rounded-t-md cursor-pointer whitespace-nowrap"
              >
      ...
      <AnimatePresence>
        {isFilterMenuOpen && (
          <motion.div
            ...
            className="border-b border-[#EDEDEB] dark:border-[#2D2D2D] bg-[#F7F7F5] dark:bg-[#202020] px-6 py-2.5 flex flex-wrap items-center gap-4 text-[12.5px]"
          >
            <div className="flex items-center gap-2">
              <span className="text-[#787774] font-medium">Area:</span>
              <select
                value={selectedContainerFilterId}
                onChange={(e) => handleSelectContainerFilter(e.target.value)}
                className="bg-white dark:bg-[#2A2A2A] border border-[#DDD] dark:border-[#444] rounded px-2 py-0.5 text-[#37352F] dark:text-[#EEE] outline-none"
              >
                <option value="all">All Areas ({nodes.filter(n => !n.isContainer && !n.archived).length})</option>
                <option value="no-container">No Area</option>
                {nodes.filter(n => n.isContainer && !n.archived).map(c => (
            ...
            <div className="flex items-center gap-2">
              <span className="text-[#787774] font-medium">Subtasks:</span>
              <button
                type="button"
                onClick={() => setShowSubtasks(!showSubtasks)}
                className={`px-2 py-0.5 rounded border text-[12px] font-medium cursor-pointer ${
                  showSubtasks ? 'bg-[#DDEDE0] text-[#2B6E44] border-[#C6E2CB]' : 'bg-white dark:bg-[#2A2A2A] border-[#DDD] text-[#787774]'
                }`}
              >
                {showSubtasks ? '✓ Shown' : 'Hidden'}
              </button>
            </div>
            ...
            <button
              type="button"
              onClick={() => {
                setCollapseCompleted(!collapseCompleted);
              }}
              className="px-2 py-0.5 rounded border border-[#DDD] dark:border-[#444] bg-white dark:bg-[#2A2A2A] text-[#787774] text-[12px] font-medium hover:text-[#37352F] cursor-pointer"
            >
