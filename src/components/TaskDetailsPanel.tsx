...
export default function TaskDetailsPanel({
  ...
  return (
    <>
      ...
      {/* Notion Sync Control Section */}
      {activeTab === 'details' && (() => {
        const apiKey = localStorage.getItem('milli_notion_api_key');
        const dbId = localStorage.getItem('milli_notion_database_id');
        if (!apiKey || !dbId) return null;

        const isSynced = !!node.notionPageId;
        const pageUrl = isSynced ? `https://notion.so/${node.notionPageId!.replace(/-/g, '')}` : null;

        return (
          <div className="mx-4 mb-4 p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <span>Notion Синхронизация</span>
              </div>
              {isSynced && (
                <span className="text-[9px] bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 px-1.5 py-0.5 rounded-full font-bold">
                  Синхронизировано
                </span>
              )}
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={async () => {
                  try {
                    let schema = null;
                    try {
                      const rawSchema = localStorage.getItem('milli_notion_schema');
                      if (rawSchema) schema = JSON.parse(rawSchema);
                    } catch {}

                    const { createNotionPage, updateNotionPage } = await import('../lib/notionService');
                    
                    if (isSynced) {
                      const res = await updateNotionPage(node, apiKey, node.notionPageId!, schema);
                      if (res.success) {
                        alert('Задача успешно обновлена в Notion!');
                      } else {
                        alert(`Ошибка обновления: ${res.error}`);
                      }
                    } else {
                      const res = await createNotionPage(node, apiKey, dbId, schema);
                      if (res.success && res.pageId) {
                        onUpdateNode({
                          ...node,
                          notionPageId: res.pageId
                        });
                        alert('Задача успешно добавлена в Notion!');
                      } else {
                        alert(`Ошибка добавления: ${res.error}`);
                      }
                    }
                  } catch (err: any) {
                    alert(`Ошибка API: ${err.message || err}`);
                  }
                }}
                className="flex-1 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg text-[11px] cursor-pointer"
              >
                {isSynced ? 'Обновить в Notion' : 'Выгрузить в Notion'}
              </button>
              ...
              {pageUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-2.5 py-1.5 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-[11px] flex items-center justify-center gap-1 cursor-pointer font-bold"
                  title="Открыть в Notion"
                >
                  <ExternalLink className="w-3.5 h-3.5" />
                  <span>Открыть</span>
