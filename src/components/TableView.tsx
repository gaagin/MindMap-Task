...
export default function TableView({
  ...
  const getSortIcon = (field: SortField) => {
    ...
    return sortOrder === 'asc' ? (
      <ChevronUp className="w-3.5 h-3.5 text-indigo-650 dark:text-indigo-400 font-medium shrink-0" />
    ) : (
      <ChevronDown className="w-3.5 h-3.5 text-indigo-650 dark:text-indigo-400 font-medium shrink-0" />
    );
  };
  ...
  const getHeaderClass = (field: SortField) => {
    ...
    return `relative px-4 py-2 cursor-pointer transition-colors border-r border-slate-200 dark:border-slate-800 ${
      isActive 
        ? 'bg-indigo-50/40 dark:bg-indigo-950/15 text-indigo-700 dark:text-indigo-400 font-medium block-border' 
    ...
    >
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-xs py-2 px-3 rounded-xl border cursor-pointer select-none transition-all outline-none ${
                isControlsExpanded 
                  ? 'bg-indigo-50 dark:bg-indigo-950/40 border-indigo-200 dark:border-indigo-900 text-indigo-600 dark:text-indigo-400 font-medium shadow-sm' 
            ...
            >
              ...
              <span className="hidden sm:inline text-xs font-medium">Фильтр / Сортировка</span>
              {isControlsExpanded ? (
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-xs py-2 px-3 rounded-xl border cursor-pointer select-none transition-all outline-none shrink-0 ${
                isFullScreen 
                  ? 'bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800 text-amber-600 dark:text-amber-400 font-medium shadow-sm w-auto' 
            ...
            >
              ...
              <span className="hidden sm:inline-block font-medium">{isFullScreen ? "Свернуть" : "На весь экран"}</span>
            </button>
          </div>
        </div>
            ...
            >
              <div className="flex flex-col sm:flex-row items-center gap-3 p-3.5">
                ...
                <div className="flex items-center gap-1 p-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 text-xs w-full sm:w-auto shadow-sm">
                  <span className="text-[10px] uppercase font-medium text-slate-400 dark:text-slate-500 pl-2 pr-1 select-none whitespace-nowrap">Сортировка:</span>
                  <select
                    value={sortField}
                    aria-label="Поле сортировки"
                    onChange={(e) => handleSort(e.target.value as SortField)}
                    className="bg-transparent border-0 text-slate-700 dark:text-slate-200 text-xs py-1.5 px-2 focus:outline-none font-medium cursor-pointer rounded-lg hover:bg-slate-100 dark:hover:bg-slate-850"
                  >
                    <option value="text" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium">По алфавиту / имени</option>
                    <option value="dueDate" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium">По дате выполнения</option>
                    <option value="priority" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium">По приоритету</option>
                    <option value="progress" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium">По прогрессу</option>
                    <option value="completed" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium">По статусу</option>
                  </select>
                  <button
                    type="button"
                  ...
                  >
                    <span className="text-[9px] font-medium uppercase mr-1">
                      {sortOrder === 'asc' ? 'А-Я' : 'Я-А'}
                    </span>
                    {sortOrder === 'asc' ? (
                      <ChevronUp className="w-3.5 h-3.5 text-indigo-650 dark:text-indigo-400 font-medium" />
                    ) : (
                      <ChevronDown className="w-3.5 h-3.5 text-indigo-650 dark:text-indigo-400 font-medium" />
                    )}
                  </button>
                </div>
                ...
                <div className="flex items-center gap-1 p-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 text-xs w-full sm:w-auto shadow-sm">
                  <span className="text-[10px] uppercase font-medium text-slate-400 dark:text-slate-500 pl-2 pr-1 select-none whitespace-nowrap">Статус:</span>
                  <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value as any)}
                    className="bg-transparent border-0 text-slate-700 dark:text-slate-200 text-xs py-1.5 px-2 focus:outline-none font-medium cursor-pointer rounded-lg hover:bg-slate-100 dark:hover:bg-slate-850"
                  >
                    <option value="active" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium">Все активные</option>
                    <option value="todo" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium">План</option>
                    <option value="progress" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium">В работе</option>
      ...
      <div className="flex-1 overflow-auto custom-scrollbar select-none bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
        ...
        >
          ...
          <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-800 h-10 shrink-0 font-medium text-[10px] uppercase tracking-wider text-slate-400 sticky top-0 z-20">
            <tr>
              ...
              <th className="group relative px-4 py-2 font-medium text-[10px] text-slate-400 dark:text-slate-500 text-left border-r border-slate-200 dark:border-slate-800" style={{ width: widths.tags }}>
                <span>Теги</span>
                {renderResizer('tags')}
              </th>
              <th className="group relative px-4 py-2 text-center font-medium text-[10px] text-slate-400 dark:text-slate-500 border-r border-slate-200 dark:border-slate-800" style={{ width: widths.options }}>
                <span>Опции</span>
                  ...
                  >
                    ...
                    <td className="px-4 py-2 border-r border-slate-200 dark:border-slate-800/80" style={{ paddingLeft: `${depth * 20 + 16}px` }}>
                      <div className="flex items-center gap-1.5 overflow-hidden w-full">
                        ...
                        {task.status === 'waiting' && !task.completed && (
                          <span className="shrink-0 inline-flex items-center gap-1 text-[9px] font-medium uppercase bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded border border-indigo-150/20" title="В ожидании">
                            ⏳ Ожидание
                          </span>
                        )}
                        <input
                          ...
                          className={`w-full bg-transparent border-0 focus:ring-0 p-1 rounded hover:bg-slate-100/50 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-100 font-medium focus:outline-none focus:bg-slate-100 dark:focus:bg-slate-800 truncate ${
                        ...
                        {task.archived && (
                          <button
                            ...
                            className="shrink-0 inline-flex items-center gap-1 text-[8.5px] font-medium uppercase text-amber-600 dark:text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 active:bg-amber-500/30 px-1.5 py-0.5 rounded border border-amber-500/20 hover:border-amber-500/40 select-none cursor-pointer hover:scale-105 transition-all"
                        ...
                        {activePomodoroNodeId === task.id && (
                          <span className="inline-flex items-center gap-1 bg-rose-500/10 text-rose-600 dark:text-rose-450 px-1 py-0.5 rounded-md text-[10px] font-sans font-medium animate-pulse ml-1 shrink-0 border border-rose-500/20 shadow-[0_0_8px_rgba(239,68,68,0.2)]" title="Запущена фокусировка Pomodoro">
                        ...
                        {(() => {
                          ...
                          return stats.pomodoroTotalTime > 0 ? (
                            <span 
                              ...
                              className="shrink-0 inline-flex items-center gap-1 text-[9px] font-medium text-rose-600 bg-rose-50 dark:bg-rose-950/30 px-1.5 py-0.5 rounded border border-rose-150/20 select-none"
                        ...
                        {task.estimatedTime !== undefined && task.estimatedTime !== null && !isNaN(task.estimatedTime) ? (
                          <span 
                            ...
                            className="shrink-0 inline-flex items-center gap-1 text-[9px] font-medium text-indigo-600 bg-indigo-50 dark:bg-indigo-950/30 px-1.5 py-0.5 rounded border border-indigo-150/20 cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors"
                          ...
                          <span 
                            ...
                            className="shrink-0 inline-flex items-center gap-1 text-[9px] font-medium text-slate-400 dark:text-slate-500 bg-slate-50/50 dark:bg-slate-800/40 border border-dashed border-slate-300 dark:border-slate-700/60 px-1.5 py-0.5 rounded cursor-pointer hover:text-indigo-600 hover:border-indigo-300 dark:hover:text-indigo-400 hover:bg-indigo-50/30 dark:hover:bg-indigo-950/20 transition-all"
                    ...
                    <td className="px-4 py-2 border-r border-slate-200 dark:border-slate-800/80">
                      <button
                        ...
                        className={`px-2 py-1.5 rounded-xl text-[10px] font-medium select-none cursor-pointer transition-all hover:scale-102 ${getPriorityBadge(task.priority)}`}
                      >
                    ...
                    <td className="px-4 py-2 border-r border-slate-200 dark:border-slate-800/80">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="font-mono text-[10.5px] font-medium text-slate-500 hover:text-indigo-600 dark:text-slate-450 dark:hover:text-indigo-400 px-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded cursor-pointer"
                        >
                          {task.progress || 0}%
                        </button>
                      </div>
                    ...
                    <td className="px-4 py-2 border-r border-slate-200 dark:border-slate-800/80">
                      <div className="flex items-center gap-1.5 font-mono text-[10.5px]">
                        <span className="text-rose-500 font-medium shrink-0">🍅</span>
                        {(() => {
                          const stats = getPomoStatsForNode(task, nodes);
                          return stats.pomodoroTotalTime > 0 ? (
                            <span 
                      ...
                      <div className="flex flex-wrap gap-1">
                        {task.tags && task.tags.length > 0 ? (
                          task.tags.map(t => (
                            <span
                              key={t}
                              className="px-1.5 py-0.5 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 rounded-md text-[9.5px] font-medium border border-indigo-100/50 dark:border-indigo-900/40"
                            >
                              #{t}
                            </span>
                          ))
                          ...
                          return (
                            <button
                              ...
                              className={`p-1 rounded transition-colors cursor-pointer flex items-center gap-0.5 text-[9.5px] ${
                                hasComments
                                  ? 'bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 font-medium px-1'
